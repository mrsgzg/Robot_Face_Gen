...
class FacialReactionModel(nn.Module):
    ...
    def __init__(
        ...
        # 历史状态处理124
        self.past_motion_linear = nn.Linear(124, feature_dim)  # 102+18+4+2 -> feature_dim
    ...
    def forward(
        ...
        past_reaction_features = torch.zeros(
            (batch_size, self.window_size, 124),  
    ...
    def inference_step(
        self,
        speaker_data: Dict[str, torch.Tensor],
        past_reaction_features: torch.Tensor,  # (B, window_size, 124)
...
class OnlineInferenceManager:
    ...
    def initialize(self, batch_size: int = 1):
        ...
        self.past_reaction_features = torch.zeros(
            (batch_size, self.model.window_size, 124),
            device=self.device
        )
        self.past_motion_sample = None
