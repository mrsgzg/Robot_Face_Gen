...
class SlidingWindowTester:
    ...
    def reset_states(self, batch_size=1):
        """重置滑动窗口状态"""
        # 🔹 修改：历史特征维度从158改为124 (102+17+3+2)
        self.past_reaction_features = torch.zeros(
            (batch_size, self.window_size, 124),  # 102(landmarks) + 17(au) + 3(pose) + 2(gaze)
            device=self.device
        )
        self.past_motion_sample = None
    ...
    def denormalize_features(self, data_dict):
        """
        反归一化特征
        🔹 修改：landmarks现在是中心化的相对坐标，需要调整处理方式
        """
        result = {}
        for key, value in data_dict.items():
            if key == 'landmarks':
                # landmarks现在是中心化的相对坐标，恢复到合理的显示尺度
                result[key] = value * 100.0  # 调整尺度因子以便可视化
            elif key == 'au':
    ...
    def save_complete_results(self, predictions, sample_data, output_dir='sliding_results'):
        ...
        def save_all_features_csv(features_dict, prefix, output_dir):
            for feature_name, feature_data in features_dict.items():
                ...
                if feature_name == 'landmarks':
                    # 🔹 修改：现在是51个点（17-67号点），102维
                    columns = [f'x_{i}' for i in range(17, 68)] + [f'y_{i}' for i in range(17, 68)]
        ...
        print(f"   注意：landmarks现在只包含17-67号特征点（51个点，102维，中心化坐标）")
    ...
    def calculate_metrics(self, pred_denorm, gt_denorm):
        ...
        for feature_name in ['landmarks', 'au', 'pose', 'gaze']:
            if feature_name in pred_denorm and feature_name in gt_denorm:
                ...
                # landmarks的详细分析
                if feature_name == 'landmarks':
                    # 🔹 修改：现在是51个点，前51维是x坐标，后51维是y坐标
                    x_errors = np.abs(pred_data[:, :51] - gt_data[:, :51])
                    y_errors = np.abs(pred_data[:, 51:] - gt_data[:, 51:])
    ...
    def create_smoothness_plots(self, frame_diffs, output_dir):
        ...
        # 关键点运动热图 (51个点)
        plt.subplot(2, 3, 6)
        point_motion = np.sqrt(frame_diffs[:, :51]**2 + frame_diffs[:, 51:]**2)
        plt.imshow(point_motion.T, aspect='auto', cmap='hot', interpolation='nearest')
        plt.title('关键点运动热图 (51点)')
        plt.xlabel('帧索引')
        plt.ylabel('关键点索引 (17-67)')
    ...
    def create_debug_static_plot(self, speaker_denorm, gt_denorm, pred_denorm, frame_indices, output_dir):
        ...
        # 🔹 修改：现在是51个点，前51维是x坐标，后51维是y坐标
        speaker_x = speaker_denorm['landmarks'][first_idx, :51]
        speaker_y = speaker_denorm['landmarks'][first_idx, 51:]
        gt_x = gt_denorm['landmarks'][first_idx, :51]
        gt_y = gt_denorm['landmarks'][first_idx, 51:]
        pred_x = pred_denorm['landmarks'][first_idx, :51]
        pred_y = pred_denorm['landmarks'][first_idx, 51:]
        
        # 创建2x2子图
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        
        # 🔹 使用相对坐标的可视化方式，因为现在是中心化坐标
        all_x = np.concatenate([speaker_x, gt_x, pred_x])
        all_y = np.concatenate([speaker_y, gt_y, pred_y])
        max_range = max(np.max(np.abs(all_x)), np.max(np.abs(all_y)))
        margin = 0.1 * max_range
        
        # 左上：三路对比（相对坐标）
        ax1 = axes[0, 0]
        ax1.set_title(f'第一帧对比 - 帧 {frame_indices[first_idx]} (相对坐标, 51点)', fontsize=14, fontweight='bold')
        ...
        self.draw_face_connections_51points(ax1, speaker_x, speaker_y, 'blue')
        self.draw_face_connections_51points(ax1, gt_x, gt_y, 'green')
        self.draw_face_connections_51points(ax1, pred_x, pred_y, 'red')
        
        ax1.set_xlim(-max_range - margin, max_range + margin)
        ax1.set_ylim(max_range + margin, -max_range - margin)
        ...
        # 绘制中心点
        ax1.scatter(0, 0, c='black', s=80, marker='+', linewidth=3)
        
        # 右上：误差分析
        ax2 = axes[0, 1]
        ax2.set_title('预测误差热图 (51点)', fontsize=14, fontweight='bold')
...
Pred:    X[{pred_x.min():.1f}, {pred_x.max():.1f}], Y[{pred_y.min():.1f}, {pred_y.max():.1f}]
    ...
    def create_comparison_animation(self, predictions, sample_data, output_path='sliding_results/sliding_comparison_300.gif'):
        ...
        for pred in predictions:
            ...
            pred_landmarks.append(landmarks * 100.0)  # 反归一化为显示尺度
            frame_indices.append(pred['end_frame'] - 1)  # 转换为0-based索引
        
        pred_landmarks = np.array(pred_landmarks)
        
        # 获取对应的真实landmarks和speaker landmarks
        gt_landmarks = sample_data['listener']['landmarks'].numpy() * 100.0
        speaker_landmarks = sample_data['speaker']['landmarks'].numpy() * 100.0
        ...
        # 🔹 修改：提取坐标，现在是51个点
        pred_x = pred_landmarks[:n_frames, :51]
        pred_y = pred_landmarks[:n_frames, 51:]
        gt_x = gt_landmarks_aligned[:n_frames, :51]
        gt_y = gt_landmarks_aligned[:n_frames, 51:]
        speaker_x = speaker_landmarks_aligned[:n_frames, :51]
        speaker_y = speaker_landmarks_aligned[:n_frames, 51:]
        ...
        def animate(frame):
            ...
            # 设置标题
            ax1.set_title(f'Speaker - 帧 {frame_indices[frame]} (51点)', fontsize=12)
            ax2.set_title(f'GT Listener - 帧 {frame_indices[frame]} (51点)', fontsize=12)
            ax3.set_title(f'滑动窗口预测 - 帧 {frame_indices[frame]} (51点)', fontsize=12)
            ...
            # 绘制面部连接线（使用51点连接关系）
            self.draw_face_connections_51points(ax1, speaker_x[frame], speaker_y[frame], 'blue')
            self.draw_face_connections_51points(ax2, gt_x[frame], gt_y[frame], 'green')
            self.draw_face_connections_51points(ax3, pred_x[frame], pred_y[frame], 'red')
...
理论延迟: {self.window_size/25*1000:.0f}ms ({self.window_size}帧@25fps)
    ...
    def draw_face_connections_51points(self, ax, x_coords, y_coords, color='blue'):
        """
        绘制51个面部特征点的连接线（17-67号点）
        🔹 新增：使用新的51点连接关系
        """
        # 定义51个点的连接关系（对应原来的17-67号点）
        # 注意：这里的索引是在51点数组中的索引
        connections = {
            # 右眉毛 (原17-21，现在数组索引0-4)
            'right_eyebrow': list(range(0, 5)),
            
            # 左眉毛 (原22-26，现在数组索引5-9)
            'left_eyebrow': list(range(5, 10)),
            
            # 鼻梁 (原27-30，现在数组索引10-13)
            'nose_bridge': list(range(10, 14)),
            
            # 鼻翼 (原31-35，现在数组索引14-18)
            'nose_tip': list(range(14, 19)),
            
            # 右眼 (原36-41，现在数组索引19-24)
            'right_eye': list(range(19, 25)) + [19],  # 闭合
            
            # 左眼 (原42-47，现在数组索引25-30)
            'left_eye': list(range(25, 31)) + [25],   # 闭合
            
            # 外嘴唇 (原48-59，现在数组索引31-42)
            'outer_lip': list(range(31, 43)) + [31],  # 闭合
            
            # 内嘴唇 (原60-67，现在数组索引43-50)
            'inner_lip': list(range(43, 51)) + [43]   # 闭合
        }
        
        # 根据颜色选择不同部位的颜色
        if color == 'blue':
            colors = ['navy', 'blue', 'steelblue', 'steelblue', 'darkblue', 'darkblue', 'royalblue', 'royalblue']
        elif color == 'green':
            colors = ['darkgreen', 'green', 'forestgreen', 'forestgreen', 'darkgreen', 'darkgreen', 'limegreen', 'limegreen']
        elif color == 'red':
            colors = ['darkred', 'red', 'crimson', 'crimson', 'darkred', 'darkred', 'orangered', 'orangered']
        else:
            colors = [color] * 8
        ...
        for i, (name, indices) in enumerate(connections.items()):
            if len(indices) > 1:
                try:
                    ...
                    ax.plot(x_line, y_line, color=colors[i % len(colors)], linewidth=1, alpha=0.6)
                except IndexError:
                    continue
...
def main():
    ...
    try:
        ...
        print(f"   Landmarks维度: {sample['speaker']['landmarks'].shape} (51个点×2坐标，中心化)")
        ...
        if args.compare_strides:
            ...
            print("="*60)
            print(f"🚀 测试配置:")
            print(f"  特征配置: 51个面部特征点 (17-67号点)")
            print(f"  坐标系统: 中心化相对坐标")
            ...
            print(f"  注意: CSV文件中landmarks列名为x_17到x_67, y_17到y_67")
            ...
            summary = {
                'config': {
                    'feature_points': 51,
                    'coordinate_system': 'centered_relative',
                    'point_range': '17-67',
                    'stride': args.stride,
